    ...
    <div class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-black bg-opacity-50"
        ...
        <div class="!w-3/4 px-6 py-4 mx-auto text-left bg-white dark:bg-gray-800 rounded shadow-lg"
            ...
            <div class="container px-6 mx-auto grid">
                ...
                @endif
                <form action="{{ route('adminprofile.update', [$masyarakat->id]) }}" method="post"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4 mr-4">
                            ...
                            {{-- <input type="number" name="nik" id="nik" value="{{ $masyarakat->nik }}"
                            ...
                            <input type="number"
                                ...
                                value="{{ $masyarakat->nik }}" class="mt-1 p-2 border rounded-md w-full" name="nik"
                        ...
                        <div class="mb-4 mr-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ $masyarakat->name }}"
                                class="mt-1 p-2 border rounded-md w-full">
                        </div>
                        <div class="mb-4 mr-4">
                            <label for="email"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-200">E-mail:</label>
                            <input type="email" name="email" id="email" value="{{ $masyarakat->email }}"
                                class="mt-1 p-2 border rounded-md w-full">
                        </div>
                        <div class="mb-4 mr-4">
                            <label for="phone"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-200">Phone:</label>
                            {{-- <input type="number" name="phone" id="phone" value="{{ $masyarakat->phone }}"
                                class="mt-1 p-2 border rounded-md w-full"> --}}
                            <input type="number" pattern="^[0-9]{12}$"
                                oninput="if(this.value.length > 12) this.value = this.value.slice(0, 12)" name="phone" value="{{ $masyarakat->phone }}"
