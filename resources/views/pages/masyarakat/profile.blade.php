...
@section('content')
    <main class="h-full pb-16 overflow-y-auto p-4">
        ...
        <form action="{{ route('profile.update', [$data->uuid]) }}" method="post" class="max-w-md mx-auto mt-4">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4 mr-4">
                    ...
                    <input type="number"
                        ...
                        oninput="if(this.value.length > 16) this.value = this.value.slice(0, 16)" class="mt-1 p-2 border rounded-md w-full"
                        name="nik" id="nik" placeholder="Masukkan NIK (maksimal 16 digit)" required autofocus value="{{$data->nik}}">
                </div>
                <div class="mb-4 mr-4">
                ...
                <div class="mb-4 mr-4">
                    ...
                    oninput="if(this.value.length > 12) this.value = this.value.slice(0, 12)"  name="phone" id="phone" value="{{ $data->phone }}"
                        class="mt-1 p-2 border rounded-md w-full">
                </div>
                <div class="mb-4 mr-4">
                    <label for="password"
